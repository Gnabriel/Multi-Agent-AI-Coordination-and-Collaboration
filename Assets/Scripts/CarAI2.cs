        ...
        private void FixedUpdate()
        {
            ...
            for (int f_i = 0; f_i < friends.Length; f_i++)
            {
                ...
                if (Math.Abs(friends[f_i].transform.position.x - wanted_pos.x) + Math.Abs(friends[f_i].transform.position.z - wanted_pos.z) <
                    7.5f)
                {
                    foreach (Vector3Int nod in mst_nodes_only)
                    {
                        Vector3 nod_world = get_pos(nod);
                        // nod_world = new Vector3(nod_world.x, 1.0f, nod_world.z);
                        Vector3 car_world = friends[f_i].transform.position;
                        // car_world = new Vector3(car_world.x, 1.0f, car_world.z);
                        float dist = Math.Abs(car_world.x - nod_world.x) + Math.Abs(car_world.z - nod_world.z);
                        RaycastHit[] all_hits = Physics.RaycastAll(car_world, nod_world, dist);
                        bool no_cube = true;
                        ...
                        if(no_cube){
                            mst_nodes_only.Remove(nod);
                            if (mst_only.ContainsKey(nod))
                            {
                                foreach (Vector3Int nod2 in mst_only[nod])
                                {
                                    mst_only[nod2].Remove(nod);
                                }

                                mst_only[nod] = new HashSet<Vector3Int>();
                            }
                        }
                    }
