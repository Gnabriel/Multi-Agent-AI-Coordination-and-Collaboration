...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAI3 : MonoBehaviour
    {
        ...
        public Vector3Int get_real_closest(Vector3 pos)
        {
        ...
        public Vector3Int get_closest(Vector3Int current_pos, Vector3 pos, GameObject[] enemies)
        {
            float current_dist = INF;
            Vector3Int current_node = new Vector3Int((int) pos.x, 0, (int) pos.z);
            foreach (var enemy in enemies)
            {
                Vector3Int node = get_real_closest(enemy.transform.position);
                Vector3 node_pos = get_pos(node);
                float new_dist = Math.Abs(pos.x - node_pos.x) + Math.Abs(pos.z - node_pos.z);
                if (new_dist < current_dist)
                {
        ...
        private void Start()
        {
            ...
            if (m_Car.name == friends[0].name)
            {
                ...
                {
                    Vector3Int closest_node = new Vector3Int(-1, 0, -1);
        ...
        private void FixedUpdate()
        {
            ...
            if (enemies.Length > 0)
            {
                ...
                {
                    if (f_i < enemies.Length)
                    {
                        Vector3Int current_wanted_pos = current_nodes[f_i];
                        Vector3 wanted_pos = get_pos(current_wanted_pos);

                        if (current_wanted_pos == new Vector3Int(-1, 0, -1))
                        {
                            Vector3Int new_wanted_pos = get_closest(current_wanted_pos, friends[f_i].transform.position, enemies);
                            current_nodes[f_i] = new_wanted_pos;
                            wanted_pos = get_pos(new_wanted_pos);
                            mst_nodes_only.Remove(new_wanted_pos);
                        }
                        else if (Math.Abs(friends[f_i].transform.position.x - wanted_pos.x) +
                            Math.Abs(friends[f_i].transform.position.z - wanted_pos.z) <
                            7.5f)
                        {
                            Vector3Int new_wanted_pos = get_closest(current_wanted_pos, friends[f_i].transform.position, enemies);
                            current_nodes[f_i] = new_wanted_pos;
            ...
            foreach (Vector3Int pos in adjacencies.Keys)
            {
                foreach (Vector3Int pos2 in adjacencies[pos])
                {
